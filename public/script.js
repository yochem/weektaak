...
function fillWeekList(weekList) {
  const personalLink = (name) => `<a href="/persoonlijk.html?naam=${name}">${name}</a>`;

  const linkList = [
    weekList.k1,
    weekList.k2,
    weekList.k3,
  ].map(personalLink).join(', ');

  setContent("kitchen", linkList);
  setContent("toilet", personalLink(weekList.wc));
  setContent("shower", personalLink(weekList.douches));
  setContent("weeknumber", `Week ${weekNumberFromDate(weekList.weekStart)}`);
...
function fillPersonalPage(data) {
  ...
  const params = new URLSearchParams(window.location.search);
  let person = params.get("naam");
  const option = Array.from(element.options).find(opt => opt.value === person);
  if (option) {
...
function fillPersonalTable(data, person) {
  const rows = Object.entries(data).reduce((acc, [weekStart, weekData]) => {
    const weekTasks = Object.entries(weekData)
      ...
      .map(([task, _]) => task.startsWith("k") ? "keuken" : task);

    if (weekTasks.length > 0) {
      const weekNum = weekNumberFromDate(weekStart);
      const mondayDate = new Date(weekStart).toLocaleDateString('nl-NL', {
        month: 'short',
        day: 'numeric',
      });
  ...
  document.querySelector("#icslink").setAttribute(
    'href',
    `/cal/${person.toLowerCase()}.ics`,
  );
}
...
fetch('data.json')
  ...
  .then(data => {
    ...
    if (!page || page === '/' || page === '/index.html') {
      ...
      document.getElementById("names").addEventListener("change", change => {
